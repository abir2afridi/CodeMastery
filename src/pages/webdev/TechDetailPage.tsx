...
import { getCategoryById } from "@/data/webdev/technologies";
import { useTechnologies } from "@/hooks/webdev/useTechnologies";
import { useBookmarks } from "@/hooks/webdev/useBookmarks";
import { useCompare } from "@/hooks/webdev/useCompare";
  ...
  if (!tech) {
    return (
      <div className="flex flex-col items-center justify-center min-h-[50vh] text-center px-4">
        ...
        <Button variant="link" onClick={() => navigate("/web-dev")} className="text-sm uppercase font-black text-primary mt-4">{t("tech.returnHome")}</Button>
      </div>
    );
  }
      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 pb-6 border-b border-border/40">
        <div className="space-y-4">
          ...
          <div className="flex items-center gap-3 text-[12px] font-black uppercase tracking-[0.3em] text-primary/80">
            <Link to="/web-dev" className="hover:text-primary transition-colors">{t("nav.overview")}</Link>
