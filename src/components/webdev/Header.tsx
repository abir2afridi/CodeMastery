...
import { searchTechnologies, Technology, categories, technologies } from "@/data/webdev/technologies";
import { cn } from "@/lib/utils";
import { useTheme } from "@/hooks/webdev/useTheme";
...
export function Header() {
  ...
  const getPageTitle = () => {
    ...
    if (path === "/web-dev") return t("nav.overview");
    if (path === "/web-dev/ranking") return t("nav.ranking");
    if (path === "/web-dev/roadmaps") return t("nav.roadmaps");
    if (path === "/web-dev/bookmarks") return t("nav.bookmarks");
    if (path === "/web-dev/compare") return t("nav.compare");
    if (path === "/web-dev/dev-tools") return t("nav.devTools");
    if (path === "/web-dev/settings") return t("nav.settings");
    if (path.startsWith("/web-dev/category/")) {
       const slug = path.split("/").pop();
       const cat = categories.find(c => c.slug === slug);
       return cat ? cat.name : t("header.category");
    }
    if (path.startsWith("/web-dev/tech/")) {
       const slug = path.split("/").pop();
                ...
                {results.length > 0 ? (
                  results.map((tech) => (
                    <button
                      key={tech.id}
                      onClick={() => {
                        navigate(`/web-dev/tech/${tech.slug}`);
                        setIsFocused(false);
                      }}
