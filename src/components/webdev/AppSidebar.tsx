...
import { categories, getTechsByCategory } from "@/data/webdev/technologies";
import { X, Menu, Map, BarChart3, Bookmark, Wrench, Home, GitCompare, ShieldCheck, Zap, Settings } from "lucide-react";
import { useState } from "react";
import { cn } from "@/lib/utils";
import { useLanguage } from "@/hooks/webdev/useLanguage";
...
export function AppSidebar() {
  ...
  const topNavItems = [
    { to: "/web-dev", label: t("nav.overview"), icon: Home },
    { to: "/web-dev/compare", label: t("nav.compare"), icon: GitCompare },
    { to: "/web-dev/roadmaps", label: t("nav.roadmaps"), icon: Map },
    { to: "/web-dev/ranking", label: t("nav.ranking"), icon: BarChart3 },
    { to: "/web-dev/bookmarks", label: t("nav.bookmarks"), icon: Bookmark },
    { to: "/web-dev/dev-tools", label: t("nav.devTools"), icon: Wrench },
    { to: "/web-dev/settings", label: t("nav.settings"), icon: Settings },
  ];
      ...
      <aside className={cn(
        ...
        <div className="p-6 pb-2 shrink-0">
          <div className="flex items-center justify-between">
            <Link to="/web-dev" className="flex items-center gap-2 group" onClick={() => setIsOpen(false)}>
        ...
        <div className="flex-1 overflow-y-auto scrollbar-premium px-6 py-2 space-y-6">
          <nav className="space-y-1">
            {topNavItems.map(item => {
              const wdPath = location.pathname;
              const isActive = wdPath === item.to || (item.to !== "/web-dev" && wdPath.startsWith(item.to));
              return (
                <Link
                  key={item.to}
                  to={item.to}
          ...
          <div className="flex flex-col min-h-0">
            ...
            <div className="grid gap-0.5">
              {categories.map(cat => {
                const href = cat.slug === "programming-languages" ? "/web-dev/languages" : `/web-dev/category/${cat.slug}`;
