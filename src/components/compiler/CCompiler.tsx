...
import { RotateCcw, Trash2, ExternalLink, ArrowLeft, Play, Square, Terminal, AlertTriangle } from "lucide-react";
...
const DEFAULT_C = `// Welcome to C Programming!
// This is your first C program
#include <stdio.h>

int main() {
    printf("Hello, CodeMastery!\\n");
...
export function CCompiler({ initialCode }: Props) {
  const navigate = useNavigate();
  const { t } = useI18n();
  const [code, setCode] = useState(initialCode ?? DEFAULT_C);
  ...
  useEffect(() => {
    ...
    script.onerror = () => setError("Failed to load JSCPP compiler");
        ...
        <div className="container flex items-center justify-between py-4">
          <div className="flex items-center gap-4">
            ...
            <div className="flex items-center gap-2">
              <div className="w-3 h-3 rounded-full bg-[#A8B9CC]" />
      ...
      <div className="flex-1 container py-6">
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 h-[calc(100vh-200px)]">
          ...
          <div className="flex flex-col border border-border rounded-lg overflow-hidden">
            <div className="bg-card border-b border-border px-4 py-2 flex items-center justify-between">
              <div className="flex items-center gap-2">
                <Terminal className="h-4 w-4 text-[#A8B9CC]" />
                <span className="text-xs font-mono uppercase tracking-widest">main.c</span>
              </div>
            ...
            <div className="border-b border-border bg-card/50">
              <div className="px-4 py-2 flex items-center gap-2">
                <span className="text-[10px] font-mono uppercase tracking-widest text-muted-foreground">
                  stdin input (for scanf)
                </span>
              </div>
              <textarea
                value={input}
                onChange={(e) => setInput(e.target.value)}
                placeholder="Enter input values (one per line for scanf)..."
        ...
        <div className="mt-6 flex items-center justify-center gap-6 text-xs text-muted-foreground">
          <div className="flex items-center gap-2">
            ...
            <span>JSCPP v{/* version */"latest"}</span>
          </div>
          <div className="flex items-center gap-2">
            <span>C17 Standard</span>
          </div>
          <div className="flex items-center gap-2">
            <span>Supports printf, scanf, pointers, structs</span>
          </div>
        </div>
      </div>
    </div>
